                                                ...
                                                <div class="card-body">
                                                    <div class="media"><img class="img-40 img-fluid m-r-20" src="{{url('/assets/images/ticket/anthropology.png')}}" alt="">
                                                        <div class="media-body">
                                                            <h6 class="f-w-600">
                                                                <a href="@if($ticket->ssp_ticket_status >= 2) {{route('user.ticketData.index', $ticket->id)}} @endif">{{$ticket->ssp_ticket_job_title}}</a>
                                                    ...
                                                    @if($ticket->ssp_ticket_status == 1)
                                                        <span class="btn btn-secondary" style="cursor: default;"></i>Waiting Admin Uploading CSV Data</button>
                                                    @endif
                                                    @if($ticket->ssp_ticket_status != 1)
                                                        <a href="{{route('user.ticketData.index', $ticket->id)}}"><button type="button" class="btn btn-outline-primary"></i>Show</button></a>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
...
<script>
    ...
    function createTicket(){
        swal.fire({
            ...
            preConfirm: (login) => {  
                ...
                return $.ajax({
                    type: "POST", 
                    url: "{{route('user.ticketData.store')}}",
                    processData: false,
                    contentType: false,
                    cache: false,
    ...
    function getTicketData(ticketId){
        // console.log()
        link = "{{route('user.ticketData.getEdit', ':ticketId')}}";
    ...
    function updateTicket(){
        link = "{{route('user.ticketData.update', ':ticketId')}}";
    ...
    function deleteTicketData(ticketId, ticketTitle){
        link = "{{route('user.ticketData.destroy', ':id')}}";
		...
		swal.fire({
            ...
            if(result.value){
                ...
                .then(function(){ 
                    window.location.href = "{{ route('user.ticketsList.index')}}";
                });
            }
        })
    }
